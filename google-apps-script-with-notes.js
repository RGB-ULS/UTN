function doPost(e) {
  try {
    ...
    if (!sheet) {
      ...
      // Add headers to the new sheet (NOW WITH NOTES COLUMN)
      const headers = [
      ...
      console.log('Added headers to new Leads sheet with Notes column');
    }
    
    console.log('USING SHEET:', sheet.getName(), '- NOT Sheet1!');
    
    // Prepare the row data - handle both old and new field names (NOW WITH NOTES)
    const rowData = [
      ...
      data.notes || '', // NEW NOTES COLUMN
      data.timestamp || new Date().toISOString()
    ];
    
    console.log('Prepared row data with Notes:', rowData);
    ...
    console.log('=== SUCCESS - DATA ADDED TO LEADS SHEET WITH NOTES ===');
    
    // Return success response
    return ContentService
      .createTextOutput(JSON.stringify({
        success: true,
        message: 'Data successfully added to Leads sheet with Notes column',
...
// Test function to verify the script works with LEADS sheet and Notes
function testScript() {
  console.log('=== TESTING GOOGLE APPS SCRIPT - LEADS SHEET WITH NOTES ===');
  ...
  try {
    ...
    console.log('Row data with Notes:', rowData);
    console.log('Sheet now has', lastRow, 'rows');
    console.log('CONFIRMED: Writing to LEADS sheet with Notes column');
...
// Function to force create Leads sheet with headers including Notes
function createLeadsSheet() {
  console.log('=== CREATING LEADS SHEET WITH NOTES ===');
  ...
  try {
    ...
    // Add headers including Notes column
    const headers = [
      'License Plate', 'Store', 'Time', 'Lead Type', 'Rep Email',
      'First Name', 'Last Name', 'Email', 'Phone Number', 'Zip Code',
      'Image URL', 'Notes', 'Timestamp'
    ...
    console.log('Leads sheet created successfully with headers including Notes');
...
// Function to check current headers and add Notes column if missing
function updateSheetHeaders() {
  console.log('=== UPDATING SHEET HEADERS TO INCLUDE NOTES ===');
  ...
  try {
    ...
    if (currentHeaders.length !== expectedHeaders.length || 
      ...
      console.log('Headers need updating. Setting new headers...');
